...
contract MasterChef is Ownable {
    ...
    function transferTokenOwnership(address _newContractTokenOwner)
        external
        onlyOwner
    {
        require(
            _newContractTokenOwner != address(0),
            "new owner address invalid"
        );
        require(
            cake.owner() == address(this),
            "this master chef is not own token contract"
        );
        cake.transferOwnership(_newContractTokenOwner);
    }
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 cakeReward = cakePerBlock.mul(pool.allocPoint).div(
                totalAllocPoint
            );
            accCakePerShare = accCakePerShare.add(
                cakeReward.mul(1e18).div(lpSupply)
            );
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accCakePerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeCakeTransfer(msg.sender, pending);
